<?php
	...
	$select="select a.id,a.id_menu,m.nome as menu,a.nome,a.ocultar from album_videos a left join menus m on (m.id=a.id_menu) where (true) ";
	$sql="";
	$resultado=null;
	...
	if($acao=="salvar"){
		if($id==""){
		    ...
		    if($id_menu=="")
			    $sql="INSERT INTO album_videos (id_menu,nome,ocultar) VALUES (null, :nome, :ocultar )";
			else
			    $sql="INSERT INTO album_videos (id_menu,nome,ocultar) VALUES (:id_menu, :nome, :ocultar )";
			...
			try { 
				...
				$sql="$select and (a.id=(select max(id) from album_videos)) ";
				try { 
					$my_Insert_Statement = $my_Db_Connection->prepare($sql);
					if ($my_Insert_Statement->execute()) {
					  //echo "New record created successfully";
					...
					$json_saida = json_encode(
						array(
							"mensagem_sucesso"=>"albúm de videos cadastrado com sucesso!"
							,"campos"=>$campos
							,"registros"=>$lista
						)
					);
		    ...
		    if($id_menu=="")
			    $sql="update album_videos set id_menu=null,nome=:nome,ocultar=:ocultar where(id=:id)";
			else
			    $sql="update album_videos set id_menu=:id_menu,nome=:nome,ocultar=:ocultar where(id=:id)";
			...
			try { 
				...
				try { 
					...
					$json_saida = json_encode(
						array(
							"mensagem_sucesso"=>"albúm de videos atualizado com sucesso!"
							,"campos"=>$campos
							,"registros"=>$lista
						)
					);
		...
		$sql="delete from album_videos  where (album_videos.id=:id)";
		try { 
			$my_Insert_Statement = $my_Db_Connection->prepare($sql);
			$my_Insert_Statement->bindParam(":id", $id);
			if ($my_Insert_Statement->execute()) {
			...
			try { 
				...
				$json_saida = json_encode(
					array(
						"mensagem_sucesso"=>"albúm de videos excluído com sucesso!"
						,"campos"=>$campos
						,"registros"=>$lista
					)
				);
		...
		$modelo="menus";
		$sql="select $chave,$valor from $modelo where id_menu is null and menus.nome='Vídeos'";
		try { 
			$my_Insert_Statement = $my_Db_Connection->prepare($sql);
			if ($my_Insert_Statement->execute()) {
			  //echo "New record created successfully";
