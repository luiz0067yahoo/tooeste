		...
		if (data!==undefined){
			...
			for(posicao_registro in data.registros){
				...
				for(var posicao_valor in registro){
    					...
    					if (comeco==0){
    						if($("[name="+_nome+"]").is("input:checkbox")){
    						    ...
    						    if(tipos[posicao_valor].indexOf("youtube")>=0)
    						        _valor='https://www.youtube.com/embed/'+_valor;
    							campo_formulario.val(_valor);
    						}
    					}
					}
					...
					else if(tipos[posicao_valor].indexOf("blob")>=0){
					    //console.log(_nome);
					    
						linha.append($('<td class="col">').html("<div style='height:130px;overflow:hidden;'>"+_valor+"</div>"));
					}	
	...
	$.ajax(
		{
			...
			processData: false,
			contentType: false,			
            method: $(form_atual).attr("method").toUpperCase(),			
			data:$(form_atual).attr("method").toUpperCase()=="POST"?(new FormData(form_atual)):($(form_atual).serialize()),
			beforeSend: function() {
                $('#loader').show();
            },
            complete: function() {
