    ...
    function decodeTags (html) {
        ...
        switch (typeof html) {
            case 'number':
                break;
            case 'string':
                html = String(html).replace(/&gt;/g, ">").replace(/&lt;/g, "<").replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/&quot;/g, '"');
                break;
            case 'object':
                // does not modify original object, but creates a copy
                if (Array.isArray(html)) {
...
(function ($) {
    ...
    w2popup = {
        ...
        open: function (options) 
        {
            ...
            var options = $.extend({}, this.defaults, { title: '', body : '', buttons: '' }, options, { maximized: false });
            ...
            if (options.onKeydown) w2popup.onCallBack = options.onCallBack;
...

            {
                ...
                var my_id='w2ui-popup'+Math.floor(Math.random() * (1000 - 0 + 1));
                while($('#'+my_id).is(':empty')){
                    my_id='w2ui-popup'+Math.floor(Math.random() * (1000 - 0 + 1));
                }
                if (edata.isCancelled === true) return;
                ...
                if (options.showClose) {
                    btn += '<div class="w2ui-popup-button w2ui-popup-close" onmousedown="event.stopPropagation()" onclick="w2popup.close(this)">Close</div>';
                }
                if (options.showMax) {
                    btn += '<div class="w2ui-popup-button w2ui-popup-max" onmousedown="event.stopPropagation()" onclick="w2popup.toggle(this)">Max</div>';
                }
                if (options.showMin) {
                    btn += '<div class="w2ui-popup-button w2ui-popup-min" onmousedown="event.stopPropagation()" onclick="w2popup.minimize(this)">Min</div>';
                }
                // first insert just body
                 msg = '<div id="'+my_id+'" class="w2ui-popup" style="opacity: 0; left: '+ left +'px; top: '+ top +'px;'+
                          '     width: ' + parseInt(options.width) + 'px; height: ' + parseInt(options.height) + 'px; '+
                          '     min-width: ' + parseInt(options.minwidth) + 'px; min-height: ' + parseInt(options.minheight) + 'px; '+
                              w2utils.cssPrefix('transform', 'scale(0.8)', true) + ' onClick=""></div>';
                parts=$(msg);
                ...
                // then content
                msg =   
                        '<div class="linhe_resize">'+
                            '<div class="resize_left_top"></div>'+
                            '<div class="resize_top"></div>'+
                            '<div class="resize_right_top"></div>'+
                        '</div>'+             
                        '<div class="w2ui-popup-title" style="'+ (!options.title ? 'display: none' : '') +'" onmousedown="event.stopPropagation()" ondblclick="w2popup.toggle(this)">' + 
                            '<div class="resize_left"></div>'+
                            '<img class="icon_32" src="'+options.icon+'"/>'+
                            '<div class="title"></div>'+
                            '<div class="resize_right"></div>'+ 
                            btn +
                        '</div>'+
                        '<div class="resize_linhe_container">'+
                                '<div class="resize_left"></div>'+
                                '<div class="resize_center_container">'+
                                    '<div class="w2ui-box" >'+
                                        '<div class="w2ui-popup-body" ' + 
                                           (!(options.buttons) ? ' style="height:calc(100% - 40px);'+options.style+'"' : '')+ '">' +
                                        '</div>'+
                                    '</div>'+                              
                                    '<div class="line-button" style="'+ (!options.buttons ? 'display: none' : '') +'">'+
                                    '</div>'+
                                '</div>'+
                                '<div class="resize_right"></div>'+
                        '</div>'+
                            '<div class="linhe_resize">'+
                                '<div class="resize_left_bottom"></div>'+
                                '<div class="resize_bottom"></div>'+
                                '<div class="resize_right_bottom"></div>'+
                            '</div>'+
                          '<input class="w2ui-popup-hidden" style="position: absolute; top: -100px"/>'; // this is needed to keep focus in popup
                parts.html(msg);
                parts.data('options', options);               
                //parts.attr('options', options+"");               

                if (options.title) parts.find('.title').append(options.title);
                if (options.buttons) parts.find('.line-button').append(options.buttons);
                if (options.body) parts.find('.w2ui-popup-body').append(options.body);
                if ((options.url!=null) && (options.url!='')) 
                    parts.find('.w2ui-popup-body').append(
                        '<form onsubmit="return false;">'+
                            '<div class="line" style="height:30px;">'+
                                '<input type="text" name="url" style="width:calc(100% - 105px);" value="'+options.url+'" onKeypress="w2popup.pressEnterGo(this,event)">'+
                                '<input type="button" style="width:35px;" value="IR" onKeydown="this.click" onclick="w2popup.go(this)">'+
                                '<input type="button" style="width:35px;" value="<"  onKeydown="this.click" onclick="w2popup.back(this)">'+
                                '<input type="button" style="width:35px;" value=">"  onKeydown="this.click" onclick="w2popup.forward(this)">'+
                            '</div>'+
                        '</form>'+
                        '<iframe class="iframe" src="'+options.url+'" width=100% style="height:calc(100% - 35px);" frameborder=0 onload="w2popup.iframeAddCode(this)"></iframe>'
                    );
                try{
                    $(parent.document.getElementsByTagName('body')[0]).append(parts);
                }
                catch(e){$('body').append(parts);}
                ...
                // clean transform
                setTimeout(function () {
                    parts.css(w2utils.cssPrefix('transform', ''));
                    // event after
                    w2popup.status = 'open';
                    setTimeout(function () {
                        obj.trigger($.extend(edata, { phase: 'after' }));
            ...
            if(options.modal){
                this.lockScreen(options,parts);
            }
            w2popup.initialize(parts);
            this.x_w2ui_popup=parts;
            return this;
        },
        initialize: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup'); 
            var options = x_w2ui_popup.data('options');
            x_w2ui_popup.css('z-index', 16000);

            try{
                options._last_focus = $(':focus');
            }catch(e){
                options = $.extend({}, this.defaults, { title: '', body : '', buttons: '' }, { maximized: false });
                x_w2ui_popup.data('options',options);
                options._last_focus = $(':focus');
            }
            try{
                if (options.keyboard==true) $(document).on('keydown', this.keydown);
            }catch(e){}


            $('.w2ui-popup .w2ui-popup-title').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup'); 
                if (!w2popup.get(x_w2ui_popup).maximized){
                    tmp.moved=true;
                    w2popup.status = 'moving';
                    mvStart(event);
                }                     
            });
            $('.w2ui-popup .resize_right_bottom').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup'); 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_right_bottom(event);
            });
            $('.w2ui-popup .resize_left_bottom').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup');                 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_left_bottom(event);
            });
            $('.w2ui-popup .resize_right_top').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup'); 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_right_top(event);
            });
            $('.w2ui-popup .resize_left_top').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup');                 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_left_top(event);
            });
            $('.w2ui-popup .resize_top').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup');                 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_top(event);
            });
            $('.w2ui-popup .resize_bottom').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup');                 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_bottom(event);
            });
            $('.w2ui-popup .resize_left').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup');                 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_left(event);
            });
            $('.w2ui-popup .resize_right').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup');                 
                if (!w2popup.get(x_w2ui_popup).maximized==true) resize_right(event);
            });
            $('.w2ui-popup *').on('mousedown', function (event) {
                var x_w2ui_popup=$(event.target).closest('.w2ui-popup');
                w2popup.activewindows(x_w2ui_popup);
            });
            function mvStart(evnt) {

                var mouse_x = evnt.screenX;
                var mouse_y = evnt.screenY;
                
                if (!evnt) evnt = window.event;
                var x_w2ui_popup=$(evnt.target).closest('.w2ui-popup');
                
                tmp.id = x_w2ui_popup.attr('id');
                tmp.isLocked = x_w2ui_popup.find('.w2ui-lock').length == 1 ? true : false;

                tmp.x = evnt.screenX;
                tmp.y = evnt.screenY;

                tmp.pos_x = x_w2ui_popup.position().left;
                tmp.pos_y = x_w2ui_popup.position().top;
            ...
            function resize_right_bottom (evnt){
                tmp.resizing=true;
                tmp.type_resizing='right_bottom';
                w2popup.status = 'resizing';
                mvStart(evnt);
            }
            function resize_right_top (evnt){
                tmp.resizing=true;
                tmp.type_resizing='right_top';
                w2popup.status = 'resizing';
                mvStart(evnt);
            }
            function resize_left_bottom (evnt){
                tmp.resizing=true;
                tmp.type_resizing='left_bottom';
                w2popup.status = 'resizing';
                mvStart(evnt);
            }
            function resize_left_top (evnt){
                tmp.resizing=true;
                tmp.type_resizing='left_top';
                w2popup.status = 'resizing';
                mvStart(evnt);
            }
            function resize_top (evnt){
                tmp.resizing=true;
                tmp.type_resizing='top';
                w2popup.status = 'resizing';
                mvStart(evnt);
            }
            function resize_right (evnt){
                tmp.resizing=true;
                tmp.type_resizing='right';
                w2popup.status = 'resizing';
                mvStart(evnt);
            }
            function resize_left (evnt){
                tmp.resizing=true;
                tmp.type_resizing='left';
                w2popup.status = 'resizing';
                mvStart(evnt);
            }
            function resize_bottom(evnt){
                tmp.resizing=true;
                w2popup.status = 'resizing';
                tmp.type_resizing='bottom';
                mvStart(evnt);
            }
            var tmp = {
                resizing : false,
            ...
            tmp.mvMove=function(evnt) {
                if (tmp.moved) 
                {
                    var x_w2ui_popup=$('#'+tmp.id);
                    var limit_left=$(window).width()-x_w2ui_popup.width();
                    var left=tmp.pos_x + tmp.div_x;
                    left=left>0?left:0 ;
                    left=left<limit_left?left:limit_left;                    

                    var limit_top=$(window).height()-x_w2ui_popup.height()-40;
                    var top=tmp.pos_y + tmp.div_y
                    top=top>0?top:0;
                    top=top<limit_top?top:limit_top;                    
                    ...
                    var options = x_w2ui_popup  .data('options');
                    try{
                        tmp.min_width=x_w2ui_popup.css('min-width');
                        tmp.min_width=tmp.min_width.substr(0,tmp.min_width.length-2);
                        tmp.min_width=eval(tmp.min_width);
                    }catch(e){tmp.min_width=0;}
                    var limit_left=x_w2ui_popup.position().left + tmp.div_x;
                    var left=x_w2ui_popup.position().left + tmp.div_x;
                    left=left>0?left:0 ;
                    left=left<limit_left?left:limit_left; 
                    tmp.width=x_w2ui_popup.width();                   

                    try{                        
                        tmp.min_height=x_w2ui_popup.css('min-height');
                        tmp.min_height=tmp.min_height.substr(0,tmp.min_height.length-2);
                        tmp.min_height=eval(tmp.min_height);
                    }catch(e){tmp.min_height=0;}
                    var limit_top=$(window).height()-x_w2ui_popup.height()-40;
                    var top=x_w2ui_popup.position().top + tmp.div_y;
                    top=top>0?top:0;
                    top=top<limit_top?top:limit_top;
                    tmp.height=x_w2ui_popup.height();

                    if(tmp.type_resizing.indexOf("left")!=-1){
                        if(left>0)
                            tmp.width=x_w2ui_popup.width()-tmp.div_x;
                        if(tmp.width>tmp.min_width){
                            x_w2ui_popup.css('left', left+ 'px');
                        }
                        else
                            tmp.mvStop(evnt);                        
                    }
                    else if(tmp.type_resizing.indexOf("right")!=-1){
                        tmp.width=x_w2ui_popup.width()+tmp.div_x;
                    }                     

                    if(tmp.type_resizing.indexOf("top")!=-1){
                        if((x_w2ui_popup.height()-tmp.div_y)>=tmp.min_height){
                            if(top>0)
                                tmp.height=x_w2ui_popup.height()-tmp.div_y;
                            if(options.bottomFixed!=true)
                                x_w2ui_popup.css('top', top+ 'px');
                        }
                        
                        else  tmp.mvStop(evnt);
                    }
                    else if(tmp.type_resizing.indexOf("bottom")!=-1){
                        tmp.height=x_w2ui_popup.height()+tmp.div_y;
                    }  
                    x_w2ui_popup.width(tmp.width);
                    x_w2ui_popup.height(tmp.height);
                }
                else
                    return;
            }

            tmp.mvStop=function(evnt){
                if (tmp.moved) 
                {
                    if (!evnt) evnt = window.event;
                    w2popup.status = 'open';
        ...
        keydown: function (evnt) {
            ...
            // trigger event
            var edata = w2popup.trigger({ phase: 'before', type: 'keydown', target: 'popup', options: options, originalEvent: evnt });
            if (edata.isCancelled === true) return;
            // default behavior
            switch (evnt.keyCode) {
                /*case 27:
                    evnt.preventDefault();
                    if (x_w2ui_popup.find('.w2ui-message').length > 0) w2popup.message(); else w2popup.close(evnt.target);
                    break;*/
            }
            // event after
        ...
        close: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            var options =  x_w2ui_popup.data('options');
            try{x_w2ui_popup.unlockScreen(options,x_w2ui_popup);}catch(e){$('#w2ui-lock').remove();}
            try{  
                options.onClose(x_w2ui_popup);
            }catch(e){}
            x_w2ui_popup.remove();
        },
        activewindows:function(tag){
            //if($('#w2ui-lock').length==0)
            {
                var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
                var z = 0; //for setting the initial z-index's
                $('.w2ui-popup').each(function() { //set the initial z-index's
                    if($(this).is(x_w2ui_popup)){
                        $(this).css('z-index', 16000);
                        w2popup.activetask();
                    }
                ...
                $('.menu-container').css('z-index', 19000);
                $('.menu-container').css('display', 'none');
                x_w2ui_popup.show();          
            }  
        },
        disabledwindows:function(tag){
                var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
                var z = 0; //for setting the initial z-index's
                var _position=0;
                var _windows=$('.w2ui-popup');
                _windows.each(function(index) { //set the initial z-index's
                    if($(this).is(x_w2ui_popup)){
                        _position=z;
                    }
                    z++; 
                ...
                _position=(_position==0)?1:_position;
                _position=(_position==(_windows.length-1))?_windows.length-2:_position;
                w2popup.activewindows($(_windows.get(_position)));
                $('.menu-container').css('z-index', 19000);
                $('.menu-container').css('display', 'none');
        },
        activetask:function(){
                $('.task').each(function() { //set the initial z-index's
                    if ($('#'+$(this).attr('w2popup_id')).css('z-index')>=16000)
                    {
                        $(this).css('opacity','1');
                    }
                    else{
        ...
        iframeAddCode: function (_iframe) {
            var x_w2ui_popup=$(_iframe).closest('.w2ui-popup');
            var id=x_w2ui_popup.attr('id');
            var options=x_w2ui_popup.data('options')
            var callback=null;
            try{callback=options.onCallBack;}catch(e){}
            callback=(callback==null)?'':callback;
            if(
                ($(_iframe).contents().find('meta[name="w2popup"]').length==0)
                &&
                ($(_iframe).contents().find('meta[name="calback"]').length==0)
            )
            {                
                $(_iframe).contents().find('head').append($('<meta></meta>',{name:'w2popup',content:id}));
                $(_iframe).contents().find('head').append($('<meta></meta>',{name:'calback',content:callback.toString()}));
                $(_iframe).contents().find('*').on('mousedown', function (event) {
                    w2popup.activewindows(w2desktop.getPopup(id));
                });
            }
            setTimeout(
                function(){
                    w2popup.iframeAddCode(_iframe);                    
                }
                ,1000
            )
        }, 
        go: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            try{
                var x=eval(x_w2ui_popup.find("iframe").attr('pos'))
                x++;
                x_w2ui_popup.find("iframe").attr('pos',''+x);
                var _history=x_w2ui_popup.find("iframe").attr('history').split(",");
                _history.push(x_w2ui_popup.find("iframe").attr('src'));
                x_w2ui_popup.find("iframe").attr('history',''+_history.toString());
            }
            catch(e){
                x_w2ui_popup.find("iframe").attr('pos','1')
                x_w2ui_popup.find("iframe").attr('history',(new Array(x_w2ui_popup.find("iframe").attr('src'),tag.form.elements.url.value).toString()));
            }
            x_w2ui_popup.find("iframe").attr('src',tag.form.elements.url.value);
            return false;
        },
        pressEnterGo: function (tag,ev) {
            var keycode = (ev.keyCode ? ev.keyCode : ev.which);
            if ( event.which == 13 ){
                w2popup.go(tag);
            }
        },
        updateIframe: function (x_w2ui_popup,url) {
            var _iframe=x_w2ui_popup.find("iframe");
            var id=x_w2ui_popup.attr('id');
            var options=x_w2ui_popup.data('options')
            var callback=null;
            try{options.onCallBack}catch(e){};
            callback=(callback==null)?'':callback;
            $(_iframe).contents().find('head').append($('<meta></meta>',{name:'w2popup',content:id}));
            $(_iframe).contents().find('head').append($('<meta></meta>',{name:'calback',content:callback.toString()}));
            try{
                var x=eval(_iframe.attr('pos'))
                x++;
                _iframe.attr('pos',''+x);
                var _history=_iframe.attr('history').split(",");
                _history.push(_iframe.attr('src'));
                _iframe.attr('history',''+_history.toString());
            }
            catch(e){
                _iframe.attr('pos','1')
                _iframe.attr('history',(new Array(_iframe.attr('src'),_iframe).toString()));
            }
        ...
        back: function (tag) {
            ...
            try{
                ...
                x_w2ui_popup.find("iframe").attr('pos',''+x);
                var _history=x_w2ui_popup.find("iframe").attr('history').split(",");
                x_w2ui_popup.find("iframe").attr('src',_history[x]);
                tag.form.elements.url.value=_history[x];
            }
            catch(e){
            }
            return false;
        },
        forward: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            try{
                var x=eval(x_w2ui_popup.find("iframe").attr('pos'))
                var _history=x_w2ui_popup.find("iframe").attr('history').split(",");
                if (x>=_history.length-1) return false;
                x++;
                x_w2ui_popup.find("iframe").attr('pos',''+x);
                x_w2ui_popup.find("iframe").attr('src',_history[x]);
                tag.form.elements.url.value=_history[x];
        ...
        minimize: function(tag){
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            x_w2ui_popup.hide();
            w2popup.disabledwindows(tag);
        },

        toggle: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            //x_w2ui_popup=$('#'+$(tag).closest('.w2ui-popup').attr('id'));
            var obj = this;
            var options =  x_w2ui_popup.data('options');
            // trigger event
            ...
            if (edata.isCancelled === true) return;
            // defatul action
            if (options.maximized === true) w2popup.min(tag); else w2popup.max(tag);
            // event after
            setTimeout(function () {
                obj.trigger($.extend(edata, { phase: 'after'}));
            }, (options.speed * 1000) + 50);
            x_w2ui_popup.find('.w2ui-popup-button').show();
        },
        ...
        max: function (tag) {
            ...
            // default behavior
            w2popup.status   = 'resizing';
            options.prevSize = x_w2ui_popup.css('width') + ':' + x_w2ui_popup.css('height');
            // do resize
            w2popup.resize
            (
                10000, 
                ...
                function () {
                    ...
                    x_w2ui_popup.find('.w2ui-grid, .w2ui-form, .w2ui-layout').each(function () {
        ...
        get: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            return x_w2ui_popup.data('options');
        },
        ...
        clear: function(tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            x_w2ui_popup.find('.w2ui-popup-title').html('');
            x_w2ui_popup.find('.w2ui-popup-body').html('');
            x_w2ui_popup.find('.w2ui-popup-buttons').html('');
        },
        ...
        load: function (options,tag) {
            ...
            // load url
            var html = x_w2ui_popup.data(url);
            if (html != null) {
                popup(html, selector);
            } else {
                var obj=
                '<form onsubmit="return false;">'+
                    '<div class="line" style="height:30px;">'+
                        '<input type="text" name="url" style="width:calc(100% - 105px);" value="'+url+'" onKeypress="w2popup.pressEnterGo(this,event)">'+
                        '<input type="button" style="width:35px;" value="IR" onKeydown="this.click" onclick="w2popup.go(this)">'+
                        '<input type="button" style="width:35px;" value="<"  onKeydown="this.click" onclick="w2popup.back(this)">'+
                        '<input type="button" style="width:35px;" value=">"  onKeydown="this.click" onclick="w2popup.forward(this)">'+
                    '</div>'+
                '</form>'+
                '<iframe  src="'+url+'" width=100% style="height:calc(100% - 35px);" frameborder=0 ></iframe>';
                popup(obj, selector);
                x_w2ui_popup.data(url, obj); // remember for possible future purposes

            }
            function popup(html, selector,tag) {
                var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
                delete options.url;
                ...
                if ($('#w2ui-tmp > style').length > 0) {
                    ...
                    if (x_w2ui_popup.find('#div-style').length === 0) {
                        x_w2ui_popup.append('<div id="div-style" style="position: absolute; left: -100; width: 1px"></div>');
                    }
                    x_w2ui_popup.find('#div-style').html(style);
                }
                $('#w2ui-tmp').remove();
            }
        },

        message: function (options,tag) {
            var obj = this;
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            $().w2tag(); // hide all tags
            if (!options) options = { width: 200, height: 100 };
            var pWidth   = parseInt(x_w2ui_popup.width());
            var pHeight  = parseInt(x_w2ui_popup.height());
            options.originalWidth  = options.width;
            options.originalHeight = options.height;
            if (parseInt(options.width) < 10)  options.width  = 10;
            if (parseInt(options.height) < 10) options.height = 10;
            if (options.hideOnClick == null) options.hideOnClick = false;
            var poptions    = x_w2ui_popup.data('options') || {};
            var titleHeight = parseInt(x_w2ui_popup.find('.w2ui-popup-title').css('height'));
            if (options.width == null || options.width > poptions.width - 10) {
                options.width = poptions.width - 10;
            }
            if (options.height == null || options.height > poptions.height - titleHeight - 5) {
            ...
            var head     = x_w2ui_popup.find('.w2ui-popup-title');
            var msgCount = x_w2ui_popup.find('.w2ui-message').length;
            // remove message
            if ($.trim(options.html) === '' && $.trim(options.body) === '' && $.trim(options.buttons) === '') {
                var $msg = $x_w2ui_popup.find(' #w2ui-message'+ (msgCount-1));
                var options = $msg.data('options') || {};
                $msg.css(w2utils.cssPrefix({
                    'transition': '0.15s',
                    'transform': 'translateY(-' + options.height + 'px)'
                ...
                if (msgCount == 1) {
                    ...
                    x_w2ui_popup.find('#w2ui-message'+ (msgCount-2)).css('z-index', 1500);
                }
                setTimeout(function () {
                    var $focus = $msg.data('prev_focus');
                    $msg.remove();
                    ...
                    if (typeof options.onClose == 'function') options.onClose(tag);
                }, 150);
            } else {
                if ($.trim(options.body) !== '' || $.trim(options.buttons) !== '') {
                    options.html = '<div class="w2ui-message-body">'+ options.body +'</div>'+
                ...
                // hide previous messages
                $x_w2ui_popup.find('.w2ui-message').css('z-index', 1390);
                head.css('z-index', 1501);
                // add message
                x_w2ui_popup.find('.w2ui-box')
                    .before('<div id="w2ui-message' + msgCount + '" class="w2ui-message" style="display: none; z-index: 1500; ' +
                            ...
                            '</div>');
                x_w2ui_popup.find('#w2ui-message'+ msgCount).data('options', options).data('prev_focus', $(':focus'));
                var display = x_w2ui_popup.find('#w2ui-message'+ msgCount).css('display');
                $x_w2ui_popup.find('#w2ui-message'+ msgCount).css(w2utils.cssPrefix({
                    'transform': (display == 'none' ? 'translateY(-' + options.height + 'px)' : 'translateY(0px)')
                }));
                if (display == 'none') {
                    x_w2ui_popup.find('#w2ui-message'+ msgCount).show().html(options.html);
                    // timer needs to animation
                    setTimeout(function () {
                        x_w2ui_popup.find('#w2ui-message'+ msgCount).css(w2utils.cssPrefix({
                            'transform': (display == 'none' ? 'translateY(0px)' : 'translateY(-' + options.height + 'px)')
                    ...
                    setTimeout(function() {
                        obj.focus();
                        // has to be on top of lock
                        x_w2ui_popup.find('#w2ui-message'+ msgCount).css(w2utils.cssPrefix({ 'transition': '0s' }));
        ...
        focus: function (tag) {
            ...
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            var tmp = null;
            var pop = x_w2ui_popup;
            var sel = 'input:visible, button:visible, select:visible, textarea:visible';
            // clear previous blur
            $(pop).find(sel).off('.keep-focus');
            // in message or popup
            var cnt = x_w2ui_popup.find('.w2ui-message').length - 1;
            var msg = x_w2ui_popup.find('#w2ui-message' + cnt);
            if (msg.length > 0) {
        ...
        lock: function (msg, showSpinner,tag) {
			if(!w2popup.islock(tag)){
				var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
				var args = Array.prototype.slice.call(arguments, 0);
				args.unshift(x_w2ui_popup);
				w2utils.lock.apply(window, args);
			}
        },

        unlock: function (speed,tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            w2utils.unlock(x_w2ui_popup, speed);
        }
		,
		islock: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            return x_w2ui_popup.find('.w2ui-lock').length+x_w2ui_popup.find('.w2ui-lock-msg').length>0
        }
        ...
        lockScreen: function (options,tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            if ($('#w2ui-lock').length > 0) return false;
            if (options == null) options = x_w2ui_popup.data('options');
            if (options == null) options = {};
        ...
        unlockScreen: function (options,tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            if ($('#w2ui-lock').length === 0) return false;
            if (options == null) options = x_w2ui_popup.data('options');
            if (options == null) options = {};
        ...
        resizeMessages: function (tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            var obj = this;
            var options = x_w2ui_popup.data('options');
            // see if there are messages and resize them
            x_w2ui_popup.find('.w2ui-message').each(function () {
                var moptions = $(this).data('options');
                var $popup   = x_w2ui_popup;
        ...
        resize: function (width, height, callBack,tag) {
            var x_w2ui_popup=$(tag).hasClass('.w2ui-popup')?$(tag):$(tag).closest('.w2ui-popup');
            var obj = this;
            var options = x_w2ui_popup.data('options');
...
var w2alert = function (msg, title, callBack) {
    ...
    var xxx=w2popup.open({
        ...
        onClose: function () {
            if (typeof callBack == 'function') callBack(xxx.x_w2ui_popup);
        }
    }); 
    xxx.data('options').onOpen=function (event) {
        setTimeout(function () { xxx.x_w2ui_popup.find('.w2ui-popup-btn').focus(); }, 1);
    }
    xxx.data('options').onKeydown=function (event) {
        setTimeout(function () { xxx.x_w2ui_popup.find('.w2ui-popup-btn').focus().addClass('clicked');}, 1);
    }
    return {
        ok: function (fun) {
            callBack = fun;
...
var w2confirm = function (msg, title, callBack) {
    ...
    if (arguments.length == 1 && typeof msg == 'object') {
        xxx=$.extend(options, defaults, msg);

    } else {
        if (typeof title == 'function') {
            xxx=$.extend(options, defaults, {
                msg     : msg,
                callBack: title
            })
        } else {
    ...
    if (w2popup.status != 'closing' && w2popup.get()) {
        ...
        xxx.data('options').onOpen=function () {
            $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-message .w2ui-btn').on('click.w2confirm', function (event) {
                w2popup._confirm_btn = event.target.id;
                w2popup.message();
            });
        };
        xxx.data('options').onClose=function () {
            $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-message .w2ui-btn').off('click.w2confirm');
            setTimeout(function () {
        ...
        xxx.data('options').onOpen= function (event) {
            ...
            setTimeout(function () {
                $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn').on('click', function (event) {
                    w2popup.close();
                ...
                $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn#Yes').focus();
            }, 1);
        };
        xxx.data('options').onKeydown= function (event) 
        {
            // if there are no messages
            if ($('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-message').length === 0) {
                switch (event.originalEvent.keyCode) {
                    case 13: // enter
                        $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn#Yes').focus().addClass('clicked'); // no need fo click as enter will do click
                        w2popup.close();
                        break;
                    case 27: // esc
                        $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn#No').focus().click();
                        w2popup.close();
                        break;
                }
            }
...
var w2prompt = function (label, title, callBack) {
    ...
    if ( w2popup.status != 'closing' && w2popup.get()) {
        ...
        xxx.data('options').onOpen=function () {
            ...
            $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-message .w2ui-btn#Ok').on('click.w2prompt', function (event) {
                w2popup._prompt_value = $('#w2prompt').val();
                w2popup.message();
            });
            $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-message .w2ui-btn#Cancel').on('click.w2prompt', function (event) {
        ...
        xxx.data('options').onClose=function () {   
            ...
            $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-message .w2ui-btn').off('click.w2prompt');
        ...
        w2popup.open({
            ...
            xxx.data('options').onOpen= function (event) {
                // do not use onComplete as it is slower
                setTimeout(function () {
                    $('#'+xxx.x_w2ui_popup.attr('id')+' #w2prompt').val(options.value);
                    $('#'+xxx.x_w2ui_popup.attr('id')+' #w2prompt').w2field('text');
                    $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn#Ok').on('click', function (event) {
                        w2popup._prompt_value = $('#w2prompt').val();
                        w2popup.close();
                        if (typeof options.callBack == 'function') options.callBack(w2popup._prompt_value);
                    });
                    $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn#Cancel').on('click', function (event) {
                        w2popup._prompt_value = null;
                        w2popup.close();
                    });
                    $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn#Ok');
                    // set focus
                    setTimeout(function () { $('#w2prompt').focus(); }, 100);
                }, 1);
            };
            xxx.data('options').onKeydown= function (event) {
                ...
                if ($('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-message').length === 0) {
                    switch (event.originalEvent.keyCode) {
                        case 13: // enter
                            $('#'+xxx.x_w2ui_popup.attr('id')+' .w2ui-popup-btn#Ok').focus().addClass('clicked'); // no need fo click as enter will do click
                            w2popup.close();
                            break;
                        case 27: // esc
                            w2popup.close();
...
var path_site=self.location.href
	path_site=path_site.split("/")
	for (var i=1;i <= path_site.length;i++){
		if(path_site[i]!=""){
			acc=path_site[i];
			path_site=self.location.href.substring(0,self.location.href.indexOf(acc)+acc.length+1)
			break;
		}
	}
//##############################################################################################################################

var w2desktop=function(){};
setTimeout(
    function(){
        var options = w2popup.defaults;
        options.bottomFixed=true;        
        w2popup.initialize($('.menu-container'));
        $('.w2desktop .menu-container').data(options)
        $('.menu-container').hide();
        /*$.get( path_site+"admin/explorer?functions=web_basedir_upload", function( data ) {
            path_upload=w2utils.decodeTags(data);
        });
		$.get(path_site+'adm/functionsAjax.php?app=authenticity_token', function( data,textStatus,jqXHR) {
			authenticity_token=data
		});*/
    }
    ,1
);
w2desktop.startTime=function() {
    $('.w2desktop .clock').html(new Date().toTimeString().replace(/.*(\d{2}:\d{2}:\d{2}).*/, "$1"));
	if ($('.w2desktop .session').length>0){
		$.get("/adm/functionsAjax.php?app=session", function( data ) {
			$('.w2desktop .session').html(data);
		});
	}
    //setTimeout(w2desktop.startTime,1000);
}
setTimeout(w2desktop.startTime,1000);
w2desktop.startMenuClick=function(tag) {
    if($('#w2ui-lock').length==0){
        if($('.w2desktop .menu-container').is(':visible')){
            $('.w2desktop .menu-container').hide(); 
        }
        else{
            $('.w2desktop .menu-container').show();
        }
    }
} 
w2desktop.clicktask=function(tag) {
    if($('#w2ui-lock').length==0){
        w2popup.activetask(tag);
        var x_w2ui_popup=$(tag).attr("w2popup_id");
        var InternalWindowPopup=$('#'+x_w2ui_popup);
        if(
            InternalWindowPopup.is(':visible')
            &&
            InternalWindowPopup.css('z-index')>=16000
        ){
            InternalWindowPopup.hide(); 
        }
        else
            w2popup.activewindows(InternalWindowPopup);
    } 
} 
w2desktop.closepopup=function(tag) {
    var x_w2ui_popup=$(tag).closest('.w2ui-popup');
    $('.task').each(function() { //set the initial z-index's
        if($(this).attr('w2popup_id')==x_w2ui_popup.attr('id')){
            $(this).remove();
        }
    });
}
w2desktop.initialize=function(){
	setTimeout("w2popup.unlock(1000,w2desktop.getPopup());",1000);	
}
w2desktop.popup=function(tag,id_popup) {
    var link=$(tag).find('a');
    var icon_32=$(tag).find('.icon_32');
    var x_w2ui_popup=$(link).attr("w2popup_id");
    var InternalWindowPopup=$('#'+x_w2ui_popup);
    var callback="";
    try{
			if((link.attr('target')!=null)&&(link.attr('target')!='')){
            callback=
				"function(){"
						+"w2desktop.w2popupUpdateValueChildrenBySeletor('"+id_popup+"','"+link.attr('target')+"',data);"
				+"}";
			} 
			
    }catch(e){link.attr('callback','')}
    if((typeof x_w2ui_popup == typeof undefined || x_w2ui_popup == false)||(InternalWindowPopup.length==0)){
        var url_popup=(link.attr('href')[0]=="/")?link.attr('href').substring(1):link.attr('href');
        InternalWindowPopup=top.w2popup.open({
            icon          : icon_32.attr('src'),
            title         : link.html(),
            body          : '',
            url           : path_site+url_popup,
            buttons       : '',
            width         : (typeof link.attr('width') == typeof undefined )?$(window).width()-50:link.attr('width'),
            height        : (typeof link.attr('height') == typeof undefined )?$(window).height()-70:link.attr('height'),                
            minwidth      : 300,
            minheight     : 200,
            overflow      : 'hidden',
            color         : '#333',
            speed         : '0.3',
            opacity       : '0.8',
            modal         : (link.attr('modal')=="true"),
            bottomFixed   : false,
            showClose     : true,
            showMax       : true,
            showMin       : true,
            //onOpen        : function (event) { console.log('max');},
            onClose       : function (tag)   { w2desktop.closepopup(tag) },
            onCallBack    : callback
            //onMax         : function (event) { console.log('max'); },
            //onMin         : function (event) { console.log('min'); },
            //onKeydown     : function (event) { console.log('keydown');}
        });
        $(link).attr("w2popup_id",InternalWindowPopup.x_w2ui_popup.attr('id'));
        //if((link.attr('modal')=="true"))
        {
            var task=$('<div onclick="w2desktop.clicktask(this)" class="task" w2popup_id="'+InternalWindowPopup.x_w2ui_popup.attr('id')+'"><img class="icon_32" src="'+icon_32.attr('src')+'">'+$(link).html()+'</div>');
            $('.w2desktop .tasks').append(task)
            w2popup.activetask(task);
        }
		w2popup.lock('Carregando Sistema', true,$("#"+InternalWindowPopup.x_w2ui_popup.attr('id')));	
    }
    else{
        InternalWindowPopup.show();
    }
    $('.w2desktop .menu-container').hide(); 
}
w2desktop.getPopup=function(id){
    id = ((id==null)||(id==''))? w2desktop.getPopupID():id;
    var popup=$(parent.document.getElementById(id))
    if(popup.length==0)
        popup=$('#'+id);
    return  popup;
}
w2desktop.getPopupID=function(tag){
    if((tag==null)||(tag==''))
        return  $('head').find('meta[name="w2popup"]').attr("content");
    else
        //return $(tag).closest('w2ui-popup').attr("id");
        return $(tag).closest('html').find('meta[name="w2popup"]').attr("content");
}
w2desktop.getCalback=function(){
    return  $('head').find('meta[name="calback"]').attr("content");
}  
w2desktop.$=function(selector){
	return w2desktop.getPopup().find('.iframe').contents().find(selector)
}
w2desktop.findIframebyPopupid=function(id){
    return  w2desktop.getPopup(id).find('.iframe')
}    
w2desktop.w2popupUpdateValueChildrenBySeletor=function(id_popup,selector,value){
    w2desktop.findIframebyPopupid(id_popup).contents().find(selector).val(value);
    try{w2desktop.findIframebyPopupid(id_popup).contents().find(selector).focus();}catch(e){}
} 
var w2explorer=function(){};
w2explorer.menuEnable=function(_menu){
    var menu=$(_menu);
    menu.addClass("hover");
    menu.find('ul:first').css('visibility', 'visible');
    if($('.itens_explorer_selected').length==0){
        $('a.delete_file_folder').css('color','#AAA');
        $('a.properties_file_folder').css('color','#AAA');
    }
    else{
        $('a.delete_file_folder').css('color','');
        $('a.properties_file_folder').css('color','');
    }
}
w2explorer.menuDisable=function(_menu){
    var menu=$(_menu);
    menu.removeClass("hover");
    menu.find('ul:first').css('visibility', 'hidden');
}
w2explorer.menuSendFile=function(){
    $('#file').click();
}
w2explorer.menuCreateFolder=function(menu){
    var __url=$(menu).closest('body').find('.url').val();
	var ButtonOK=$('<button></button>',{class:'w2ui-btn',html:'Sim',tabindex:"0"});
	var ButtonCancel=$('<button></button>',{class:'w2ui-btn',html:'Não',tabindex:"0"});
	var linkButtonOK    =$('<a></a>',{href:"javascript:void(0)",id:"ok"    ,onclick:"w2explorer.AjaxMenuCreateFolder(this);"}).append(ButtonOK)
	var linkButtonCancel=$('<a></a>',{href:"javascript:void(0)",id:"cancel",onclick:"w2popup.close(this);"}).append(ButtonCancel)
    var Input=$('<input/>',{name:'folder',id:'folder',type:'text',value:'',style:'width:100%;height:30px;margin-top:20px;'});
    var Label=$('<label></label>',{html:'Digite o novo Nome:',style:'color:#555;font-family:Verdana, Arial, sans-serif;font-size:16px;'}).append(Input);
    var my_msg=$('<label></label>',{id:'my_msg',style:'color:red;font-family:Verdana, Arial, sans-serif;font-size:16px;'})
    var my_url=$('<input/>',{id:'my_url',type:'hidden',value:__url});
    var my_popup_parent=$('<input/>',{id:'my_popup_parent',type:'hidden',value:w2desktop.getPopupID(menu)});
    var body=$('<div></div>').append(Label,my_url,my_msg,my_popup_parent);
    var Buttons=$('<div></div>').append(linkButtonOK,linkButtonCancel);
    InternalWindowPopup=top.w2popup.open({
            icon          : '/adm/assets/img/oxygen-icons-png-master/oxygen/48x48/actions/folder-new.png',
    ...
    var id=InternalWindowPopup.x_w2ui_popup.attr('id');
	$('#ok').focus()
	$('#ok').keyup(function(){
		const key = event.key;
		if(key=='Spacebar' || key==' '){
			w2explorer.AjaxMenuCreateFolder(this);
		}	
	});
	$('#'+id).keyup(function(){
		const key = event.key;
    ...
    var task=$('<div onclick="w2desktop.clicktask(this)" class="task" w2popup_id="'+InternalWindowPopup.x_w2ui_popup.attr('id')+'"><img class="icon_32" src="./assets/img/oxygen-icons-png-master/oxygen/48x48/mimetypes/inode-directory.png">Criar Nova Pasta</div>');
    $('.w2desktop .tasks').append(task)
    top.w2popup.activetask(task);
    top.w2popup.activewindows($('#'+id));
    return false;
}
w2explorer.AjaxMenuCreateFolder=function(tag){
    w2popup.lock('Criando pasta', true,tag);
    $('#my_msg').val(''); 
	var _url=$('#my_url').val();
	if (_url==undefined)
		_url='';
    var input_file=w2desktop.findIframebyPopupid($('#my_popup_parent').val()).contents().find('#file');
	var fd = new FormData(input_file.get( 0 ).form);

	//fd.append('authenticity_token',authenticity_token);
	fd.append('app','explorer');
	fd.append('functions','new_folder');
	fd.append('folder',$('#folder').val());
	fd.append('url',$('#my_url').val());
    $.ajax({
		url         : '/admin/explorer',
		data        : fd,
		cache       : false,
        ...
        error: function (error) {
            w2popup.unlock(200,tag)
            $('#my_msg').html(error);
        },
        success     : function(data, textStatus, jqXHR){
            w2popup.unlock(200,tag);
            $('#my_msg').html(data);
            var id=$('#my_popup_parent').val();
            var iframe=$('#'+id).find('iframe');
            var src=iframe.attr('src');
            iframe.attr('src',src);
            if(data.indexOf('já existe')==-1){
				w2explorer.initialize()
                w2popup.close(tag);
			}
        }
    });
...
w2explorer.menuDeleteFileFolder=function(menu){
    var files_folders_selected=$(menu).closest('body').find('.itens_explorer_selected');
    if(files_folders_selected.length>0){
        file_folder=new Array();
        files_folders_selected.each(function(index) {
            file_folder.push($(this).attr('fullname'));        
        });
        if(file_folder.length>0){   
            var __url=$(menu).closest('body').find('.url').val();
            var ButtonOK=$('<button></button>',{class:'w2ui-btn',html:'Sim',tabindex:"0"});
            var ButtonCancel=$('<button></button>',{class:'w2ui-btn',html:'Não',tabindex:"0"});
			var linkButtonOK    =$('<a></a>',{href:"javascript:void(0)",id:"ok"    ,onclick:"w2explorer.AjaxMenuDeleteFolder(this);"}).append(ButtonOK)
			var linkButtonCancel=$('<a></a>',{href:"javascript:void(0)",id:"cancel",onclick:"w2popup.close(this);"}).append(ButtonCancel)
            var Label=$('<label></label>',{html:'Tem certeza que deseja Excluir :',style:'color:#555;font-family:Verdana, Arial, sans-serif;font-size:16px;'});
            var my_file_folder=$('<p></p>',{id:'my_file_folder',html:file_folder.toString(),style:'color:#555;font-family:Verdana, Arial, sans-serif;font-size:16px;word-wrap: break-word;'})
            var my_msg=$('<label></label>',{id:'my_msg',style:'color:red;font-family:Verdana, Arial, sans-serif;font-size:16px;'})
            var my_popup_parent=$('<input/>',{id:'my_popup_parent',type:'hidden',value:w2desktop.getPopupID(menu)});
            var body=$('<div></div>').append(Label,my_file_folder,my_msg,my_popup_parent);
            var Buttons=$('<div></div>');
            Buttons.append(linkButtonOK,linkButtonCancel);
            InternalWindowPopup=top.w2popup.open({
                    icon          : './assets/img/oxygen-icons-png-master/oxygen/48x48/actions/edit-delete.png',
                    title         : 'Excluir',
                    body          : body.html(),
                    url           : null,
                    buttons       : Buttons.html(),
			...
			$('#ok').keyup(function(){
				const key = event.key;
				if(key=='Spacebar' || key==' '){
					w2explorer.AjaxMenuDeleteFolder(this);
				}	
			});
            var id=InternalWindowPopup.x_w2ui_popup.attr('id');
			$('#'+id).keyup(function(){
            ...
            top.w2popup.activewindows(top.$('#'+id));
            var task=$('<div onclick="w2desktop.clicktask(this)" class="task" w2popup_id="'+InternalWindowPopup.x_w2ui_popup.attr('id')+'"><img class="icon_32" src="./assets/img/oxygen-icons-png-master/oxygen/48x48/mimetypes/inode-directory.png">Criar Nova Pasta</div>');
            top.$('.w2desktop .tasks').append(task)
            top.w2popup.activetask(task);
        }
    }
    return false;
}
w2explorer.AjaxMenuDeleteFolder=function(tag){
    w2popup.lock('Excluindo', true,tag);
	var _url=$('#my_url').val();
	if (_url==undefined)
		_url='';
    $('#my_msg').val(''); 
	...
	fd.append('authenticity_token',authenticity_token);
	fd.append('app','explorer');
	fd.append('functions','delete');
	fd.append('file_folder',$('#my_file_folder').html());
	fd.append('url',_url);
    $.ajax({
		url         : '/admin/explorer',
		data        : fd,
...
w2explorer.find=function(search){
 var parameter_url=_request.getParameter('url');
    parameter_url=((parameter_url!=null)&&(parameter_url.length!=0))?parameter_url:'';
    $('#explorer').html('');
    $('.url').val(parameter_url);
    $.get( path_site+"admin/explorer?functions=find&url="+parameter_url+'&search='+search, function( data ) {
        w2explorer.createItem(data,'list');
    });  
}
w2explorer.createItem=function(_data,type){
    _data=w2utils.decodeTags(_data);
    try{
        _data=eval(_data);
        for(var i=0;i<_data.length ;i++){
            var fileName=_data[i].file_name;
            var size=_data[i].file_size;
            var isfolder=_data[i].directory;
            var fullFileName=_data[i].full_file_name;
            var dateModify=_data[i].date_modify;
            var _width=null;
            try{_width=_data[i].width;}catch(e){}
            var _height=null;
            try{_height=_data[i].height;}catch(e){}
    
            var ext='';
            if (fileName.indexOf('.')>0) ext=fileName.split('.').pop().toLowerCase();
            var path='/adm/assets/img/oxygen-icons-png-master/oxygen/48x48/mimetypes/'
            var img='';
            if(isfolder){
                img=path+'inode-directory.png';
            }
            else if(ext=='ada'){
                img=path+'text-x-adasrc.png';
            }
            else if(ext=='c'){
                img=path+'text-x-objcsrc.png';
            }
            else if((ext=='c#')||(ext=='csharp')){
                img=path+'text-x-csharp.png';
            }
            else if(ext=='c++'){
                img=path+'text-x-c++src.png';
            }
            else if(ext=='css'){
                img=path+'text-css.png';
            }
            else if(ext=='csv'){
                img=path+'text-csv.png';
            }
            else if(ext=='dwg'){
                img=path+'image-vnd.dwg.png';
            }
            else if((ext=='doc')||(ext=='docx')){
                img=path+'application-msword.png';
            }
            else if(ext=='java'){
                img=path+'text-x-java.png';
            }
            else if(ext=='js'){
                img=path+'application-javascript.png';
            }
            else if(ext in {jpg:0,jpeg:0,png:0,gif:0,bmp:0,tif:0,svg:0}){
                img=path+'image-x-generic.png';
            }
            else if(ext=='h'){
                img=path+'text-x-objchdr.png';
            }
            else if(ext=='h++'){
                img=path+'text-x-c++hdr.png';
            }
            else if((ext=='htm')||(ext=='html')){
                img=path+'text-html.png';
            }
            else if(ext=='log'){
                img=path+'text-x-log.png';
            }
            else if(ext in {mdb:0,mdbx:0}){
                img=path+'application-vnd.ms-access.png';
            }
            else if(ext in {mp3:0,ogg:0,aac:0}){
                img=path+'audio-x-generic.png';
            }
            else if(ext in {mp4:0,avi:0,'3gp':0,flv:0,mpeg:0} ){
                img=path+'application-vnd.ms-access.png';
            }
                else if(ext == 'afm'){
                    img=path+'application-x-font-afm.png';
                }
                else if(ext == 'bdf'){
                    img=path+'application-x-font-bdf.png';
                }
                else if(ext == 'pcf'){
                    img=path+'application-x-font-pcf.png';
                }
                else if(ext == 'otf'){
                    img=path+'application-x-font-otf.png';
                }
                else if(ext == 'snf'){
                    img=path+'application-x-font-snf.png';
                }
                else if(ext == 'ttf'){
                    img=path+'application-x-font-ttf.png';
                }
            else if(ext=='pas'){
                img=path+'text-x-pascal.png';
            }
            else if(ext=='pdf'){
                img=path+'application-pdf.png';
            } 
            else if(ext=='php'){
                img=path+'application-x-php.png';
            }
            else if(ext in {ppt:0,pptx:0}){
                img=path+'application-vnd.ms-powerpoint.png';
            }
            else if(ext=='py'){
                img=path+'text-x-python.png';
            }
            else if(ext=='rtf'){
                img=path+'application-rtf.png';
            }
            else if(ext in ['rb','erb']){
                img=path+'application-x-ruby.png';
            }
            else if(ext=='svg'){
                img=path+'image-vnd.dgn.png';
            }
            else if(ext=='txt'){
                img=path+'text-x-generic.png';
            }
            else if(ext in {xls:0,xlsx:0}){
                img=path+'application-vnd.ms-excel.png';
            }
            else if(ext=='xml'){
                img=path+'text-xml.png';
            }
            else if(ext in {wave:0,wav:0,wma:0}){
                img=path+'audio-x-wav.png';
            }
            else if(ext in {zip:0,xz:0,tar:0}){
                img=path+'application-zip.png';
            }
            else{
                img=path+'application-x-zerosize.png';
                img=path+'unknown.png';
            }
            type="list";
            if(type=="icon"){
                var itens_explorer=$('<div></div>', {class: 'itens_explorer block_icon',ondblclick:(isfolder)?'w2explorer.openFolder("'+fileName+'");':'w2explorer.openFile("'+fileName+'");',onClick:'w2explorer.selectItem(this,event,"'+fileName+'")',onkeypress:'alert(e.keyCode)/*{w2explorer.AjaxMenuDeleteFolder(this)}*/'});
                itens_explorer.attr('fullname',fullFileName);
                var hint=$('<div></div>',{class: 'hint'});
                hint.append($('<p></p>',{html: 'Tamanho: '+size}));
                hint.append($('<p></p>',{html: 'Caminho: '+fullFileName}));
                hint.append($('<p></p>',{html: 'Data Modificação: '+dateModify}));
                if(_width!=null)
                    hint=$('<p></p>',{html:_width+' X '+_height});
                itens_explorer.append( $('<div></div>',{class: 'icon_itens',style:'background-image: url("'+img+'");'}).append(hint));
                itens_explorer.append( $('<div></div>', {class: 'text_itens',html:fileName}));
                $('#explorer').append(itens_explorer);    
            }
            else if(type=="list"){
                if($('#explorer').find('div').length==0){
                    var itens_explorer=$('<div></div>', {class: 'line_title_list'});
                    var div_name=$('<div></div>',{class: 'line_title_itens',style:'text-align:left;width:258px;',html:'Nome'});
                    var div_size=$('<div></div>',{class: 'line_title_itens',style:'text-align:right;width:100px;',html:'Tamanho'});
                    var div_fullName=$('<div></div>',{class: 'line_title_itens',style:'text-align:left;width:200px;',html:'Caminho Completo'});
                    var div_dateModify=$('<div></div>',{class: 'line_title_itens',style:'text-align:right;width:200px;',html:'Data Modificação'});
                    var div_img_info=$('<div></div>',{class: 'line_title_itens',style:'text-align:right;width:150px;',html:'Resolução Imagem'});
                    itens_explorer.append(div_name,div_size,div_fullName,div_dateModify,div_img_info);
                    $('#explorer').append(itens_explorer);    
                }
                var line_par="";
                if($('#explorer').find('.itens_explorer').length%2==0){
                    line_par="line_par";
                }
                var itens_explorer=$('<div></div>', {class: 'line_list itens_explorer '+line_par,ondblclick:(isfolder)?'w2explorer.openFolder("'+fileName+'");':'w2explorer.openFile("'+fileName+'");',onClick:'w2explorer.selectItem(this,event,"'+fileName+'")'});
                itens_explorer.attr('fullname',fullFileName);
                var div_icon_32=$('<div></div>',{class: 'line_icon_itens',style:'background-image: url("'+img+'");'});
                var div_name=$('<div></div>',{class: 'line_text_itens',style:'text-align:left;width:200px;',html:fileName});
                var div_size=$('<div></div>',{class: 'line_text_itens',style:'text-align:right;width:100px;',html:size});
                var div_fullName=$('<div></div>',{class: 'line_text_itens',style:'text-align:left;width:200px;',html:fullFileName});
                var div_dateModify=$('<div></div>',{class: 'line_text_itens',style:'text-align:right;width:200px;',html:dateModify});
                var div_img_info=$('<div></div>',{class: 'line_text_itens',style:'text-align:right;width:150px;'});;
                if(_width!=null)
                    div_img_info.html(_width+' X '+_height);
                itens_explorer.append(div_icon_32,div_name,div_size,div_fullName,div_dateModify,div_img_info);
                $('#explorer').append(itens_explorer);    
            }
        }
...
_request.getParameter=function(parameter){
    var url=self.location.href;
    var data=url.split('#').pop().split('&');
    var params=new Array();
    for(var i=0;i<data.length ;i++){
        var x=data[i].split('=');
        params.push({name:x[0],value:x[1]});
        if(params[i].name==parameter)
            return params[i].value;
    }
}
w2explorer.goHome=function(){
    self.location.href='#';
}
w2explorer.goPressEnter=function(event,tag){
    if (event.keyCode == 13)
        w2explorer.go($(tag).val());
}
w2explorer.go=function(url){
    url=((url!=null)&&(url.length!=0))?url:'';
    if (url!='')
        w2popup.updateIframe(w2desktop.getPopup(),url)
        //self.location.href='#url='+url;
}
w2explorer.goBack=function(tag) {
    var x_w2ui_popup=w2desktop.getPopup();
    try{
        var x=eval(x_w2ui_popup.find("iframe").attr('pos'))
        if (x<=0) return false;
        x--;
...
w2explorer.goForward =function (tag) {
    var x_w2ui_popup=w2desktop.getPopup();
    try{
        var x=eval(x_w2ui_popup.find("iframe").attr('pos'))
        var _history=x_w2ui_popup.find("iframe").attr('history').split(",");
...
w2explorer.goUP=function(){
    var parameter_url=_request.getParameter('url');
    parameter_url=((parameter_url!=null)&&(parameter_url.length!=0))?parameter_url:'';
    try{parameter_url=parameter_url.substr(0,parameter_url.length-2);}catch(e){}
    var index=parameter_url.lastIndexOf('/');
    parameter_url=parameter_url.substr(0,index);
    self.location.href='#url='+parameter_url;
}
w2explorer.initialize=function(){
    if($('#explorer').length!=0){
		w2popup.lock('Carregando Sistema', true,w2desktop.getPopup());	
        $(window).on('hashchange', function() {
            w2explorer.load('icon');
            var x_w2ui_popup=w2desktop.getPopup();
            x_w2ui_popup.find('input[name="url"]').val(self.location.href)
            try{
                var x=eval(x_w2ui_popup.find("iframe").attr('pos'));
                x++;
                x_w2ui_popup.find("iframe").attr('pos',''+x);
                var _history=x_w2ui_popup.find("iframe").attr('history').split(",");
                _history.push(self.location.href);
                x_w2ui_popup.find("iframe").attr('history',''+_history.toString());
            }
            catch(e){
                x_w2ui_popup.find("iframe").attr('pos','1')
                x_w2ui_popup.find("iframe").attr('history',(new Array(self.location.href).toString()));
            }
        })
        w2explorer.load('icon');
		w2popup.unlock(1000,w2desktop.getPopup());
    }
}
w2explorer.load=function(type){
    var parameter_url=_request.getParameter('url');
    parameter_url=((parameter_url!=null)&&(parameter_url.length!=0))?parameter_url:'';
    $('#explorer').html('');
    $('.url').val(parameter_url);
	w2popup.lock('Carregando Sistema', true,w2desktop.getPopup());	
    $.get( path_site+"admin/explorer?url="+parameter_url, function( data ) {
        w2explorer.createItem(data,type);
		w2popup.unlock(1000,w2desktop.getPopup());
    });  
    $(document).ready(function(){
        if (self.File && self.FileList && self.FileReader) {
            w2explorer.upload.Init();
        } 
    } );
}   
w2explorer.openFolder=function(url){
    var parameter_url=_request.getParameter('url');
    parameter_url=((parameter_url!=null)&&(parameter_url.length!=0))?parameter_url:'';
    url=((url!=null)&&(url.length!=0))?url:'';
    if((parameter_url!='')&&(url=='')){url=parameter_url;}
    else if((parameter_url!='')&&(url!='')){url=parameter_url+'/'+url;}
    if (url!='')
        self.location.href='#url='+url;
}
w2explorer.openFile=function(url){
    var parameter_url=_request.getParameter('url');
    parameter_url=((parameter_url!=null)&&(parameter_url.length!=0))?parameter_url:'';
    url=((url!=null)&&(url.length!=0))?url:'';
    if((parameter_url!='')&&(url=='')){url=parameter_url;}
    ...
    //url=path_upload+url;
    var x_w2ui_popup=w2desktop.getPopup();
    var CallBack = w2desktop.getCalback();
    acc_CallBack='var data="/uploads/'+url+'";x=new '+CallBack;
    acc_CallBack='var data="'+path_site+'uploads/'+url+'";x=new '+CallBack;
    try{
        CallBack = eval(acc_CallBack);
        x_w2ui_popup.find('.w2ui-popup-close').click();
    }catch(e){
        
    }
}
w2explorer.selectItem=function(tag,event,url){
    if(event.shiftKey){
        var index_tag=$(tag).index();
        var last_selected=$($('.itens_explorer_selected')[$('.itens_explorer_selected').length-1]).index();
        if(last_selected!=-1)
        $('.itens_explorer').each(function(index) {
                if(
                    (
                        (index_tag>last_selected)&&(index>=last_selected)&&(index<=index_tag)
                    )
                    ||
                    (
                        (index_tag<last_selected)&&(index>=index_tag)&&(index<=last_selected)
                    )
                    ||
                    ((index_tag==last_selected)&&(index_tag==index))
                )
                    $(this).addClass('itens_explorer_selected');
                else
                    $(this).removeClass('itens_explorer_selected');
        });
    }
    else if(event.ctrlKey){
        if($(tag).hasClass('itens_explorer_selected'))
            $(tag).removeClass('itens_explorer_selected');
        else
            $(tag).addClass('itens_explorer_selected');
    }
    else{
        if($(tag).hasClass('itens_explorer_selected')){
            $('.itens_explorer').removeClass('itens_explorer_selected');
        }
        else{
            $('.itens_explorer').removeClass('itens_explorer_selected');
            $(tag).addClass('itens_explorer_selected');
...
w2explorer.upload=function(){};
w2explorer.upload.progressByFile=function(file_name,progress){
    var div_upload=$('div.upload');
    var div_caption=null;
    var div_file=null
    div_upload.find('div.caption').each(function(){
        if ($(this).html().indexOf(file_name)>0){
            div_caption=$(this);
            div_file=div_caption.closest('div.file');
            if (div_caption.find('.progess').length==0){
                div_caption.append($('<strong></strong>',{class:'progess',html:progress}));
            }
            else{
                div_caption.find('.progess').html(progress);

            }
        }
    });
    if((div_file!=null)&&(div_caption!=null)){        
        var div_progress=div_file.find('div.progress_containner').first();
        var spam_progress=div_progress.find('.progress_blue');
        div_progress.width(progress);
    }
}
w2explorer.upload.erroByFile=function(file_name,msg){
    var div_upload=$('div.upload');
    var div_caption=null;
    var div_file=null
    div_upload.find('div.caption').each(function(){
        if ($(this).html().indexOf(file_name)>0){
            div_caption=$(this);
            div_file=div_caption.closest('div.file');
            div_caption.html('Erro ao enviar o arquivo: ');
            div_caption.append($('<strong></strong>',{html:file_name}));
            div_caption.append(' erro ');
            div_caption.append($('<strong></strong>',{html:msg}));
        }
    });
    if((div_file!=null)&&(div_caption!=null)){        
        var div_progress=div_file.find('div.progress_containner').first();
        var spam_progress=div_progress.find('.progress_blue');
            spam_progress.attr('class', 'progress_red');
            setTimeout(function(){
                if($('div.upload').find('div.caption').length<=1)
                    w2explorer.load('icon');
                div_file.remove();
                if(div_upload.find('div.file').length==0){
...
w2explorer.upload.DoneByFile=function(file_name){
    var div_upload=$('div.upload');
    var div_caption=null;
    var div_file=null
    div_upload.find('div.caption').each(function(){
        if ($(this).html().indexOf(file_name)>0){
            div_caption=$(this);
            div_file=div_caption.closest('div.file');
            div_caption.html('O arquivo: ');
            div_caption.append($('<strong></strong>',{html:file_name}));
            div_caption.append(' foi enviado por sucesso!');
        }
    });
    if((div_file!=null)&&(div_caption!=null)){        
        var div_progress=div_file.find('div.progress_containner').first();
        var spam_progress=div_progress.find('.progress_blue');
            spam_progress.attr('class', 'progress_green');
            setTimeout(function(){
                if($('div.upload').find('div.caption').length<=1)
                    w2explorer.load('icon');
                div_file.remove();
...
w2explorer.upload.addProgress=function(msg)
{        
    var div_upload=$('div.upload');
        var div_file=$('<div></div>',{class: 'file'});
            div_file.append($('<div></div>',{class: 'caption',html:msg}));
            var div_progress_containner=$('<div></div>',{class:'progress_containner'});
                div_progress_containner.append($('<span></span>',{class:'progress_blue'}));
        div_file.append(div_progress_containner);
    div_upload.append(div_file);
}
w2explorer.upload.ParseFile=function(file) {
    var msg=
        "Informações do arquivo: <strong>" + file.name +"</strong>"+
        " Tipo: <strong>" + file.type + "</strong> "+
        " Tamanho: <strong>" + file.size +"</strong> Bytes ";
    w2explorer.upload.addProgress(msg)
}
w2explorer.upload.explorerHover=function(e) {
    e.stopPropagation();
    e.preventDefault();
    e.target.className = (e.type == "dragover" ? "hover" : "");
}
w2explorer.upload.FileSelectHandler=function(e) {
    var files = e.originalEvent.target.files || e.originalEvent.dataTransfer.files;
    if(files.length>0){        
        w2explorer.upload.explorerHover(e);
        var input_file=$('#file');
        var my_form=$(input_file.get( 0 ).form);
        for (var i = 0, file; file = files[i]; i++) {
            var fd = new FormData(input_file.get( 0 ).form);
            fd.append('files[]',file,file.name);
            fd.append('authenticity_token',authenticity_token);
            var url=my_form.attr('action')+'&url='+ $('.url').val();
            w2explorer.upload.send(url,fd,file);
        }
    }
}
w2explorer.upload.FileSelectForm=function(tag) {
    var files = tag.files;
    var my_form=$(tag.form);
    if(files.length>0){
        var url=my_form.attr('action')+'&url='+ $('.url').val();
        for (var i = 0, file; file = files[i]; i++) {
            var fd = new FormData();
...
w2explorer.upload.send=function(_url,_data,file) {
    var ext=file.name.split('.').pop().toLowerCase();
    var MB=1024*1024;
    $('.upload').css('display','block');
    w2explorer.upload.ParseFile(file);
    if(ext in {jpg:0,jpeg:0,png:0,gif:0,bmp:0,tif:0,svg:0}){        
        if(file.size<5*MB){  
            $.ajax({
                url         : _url,
                data        : _data,
                ...
                xhr: function(){
                    ...
                    xhr.upload.onprogress = function(evt){
                        ...
                        setTimeout("w2explorer.upload.progressByFile('"+file.name+"','"+progress+"')",1)
                    } ;
                ...
                error: function (error) {
                    w2explorer.upload.progressByFile(file.name,'100%');
                    w2explorer.upload.erroByFile(file.name,eval(error));
                },
                success     : function(data, textStatus, jqXHR){
                    if(data.indexOf('ERRO!')>=0){
                        w2explorer.upload.progressByFile(file.name,'100%');
                        w2explorer.upload.erroByFile(file.name,data);
                    }
                    else{
                        w2explorer.upload.progressByFile(file.name,'100%');
                        w2explorer.upload.DoneByFile(file.name);
                    }
                }
            });
        }
            ...
            w2explorer.upload.progressByFile(file.name,'100%');
            w2explorer.upload.erroByFile(file.name,'ERRO! O tamanho do arquivo '+file.name+' é '+w2utils.formatSize(file.size)+' maior que o maximo permitido 5 MB');        
        }
    }
    else{
        w2explorer.upload.progressByFile(file.name,'100%')
        w2explorer.upload.erroByFile(file.name,'ERRO! A extesão do arquivo .'+ext+' não é um formato de imagem ".jpg",".jpeg",".png",".gif",".bmp",".tif",".svg" ');
    }
}
w2explorer.upload.Init=function() {
    var fileselect = $("#fileselect");
    var explorer = $("#explorer");
    var submitbutton = $("#submitbutton");
    $(document).ready(function(){
        fileselect.on("change", function(event){w2explorer.upload.FileSelectHandler(event)});
    });
    var xhr = new XMLHttpRequest();
    if (xhr.upload) {
        $(document).ready(function(){
            explorer.on("dragover", function(event){w2explorer.upload.explorerHover(event)});
            explorer.on("dragleave", function(event){w2explorer.upload.explorerHover(event)});
            explorer.on("drop", function(event){w2explorer.upload.FileSelectHandler(event)});
        });
    }
}