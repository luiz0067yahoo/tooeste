...
/* jshint ignore:start */
/* jscs:disable */
CKEDITOR_initialize=function(){
    try{
    	var popup=w2desktop.getPopup()
    	if(w2desktop.$(".cke_contents, .cke_reset").length==0){
    		w2desktop.$("textarea.ckeditor").each(function() {
    			var name=$(this).attr('name');
    			if(
    				(typeof name == typeof undefined || name == false)
    				||(name!=null)||(name!='')
    			)
    		    try{
    		    	if($(this).attr('status')!='done'){
    					w2popup.lock('Carregando Editor', true,popup);	
    		    		var id=w2desktop.$(this).attr('id');
    		    		if (
    						(typeof id == typeof undefined || id == false)
    						||(id!=null)||(id!='')
    					)
    		    			w2desktop.$(this).attr('id','textarea_ckeditor_'+ w2desktop.$("textarea.ckeditor").length);
    			    	try{
    			    		CKEDITOR.replace(w2desktop.$(this).attr('id'));
    			    	}catch(e){}		    		
    		    	}
    		    }
    		    catch(e){ }
    		...
    		w2desktop.$("textarea.ckeditor").attr('status','done');
    		w2popup.unlock(200,popup);
    	}
    	setTimeout("CKEDITOR_initialize();",3000);
	}catch(ee){ }
}
